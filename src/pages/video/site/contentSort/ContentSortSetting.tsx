...
import { delVideoSortVideoApi, getVideoAllCategoryApi, getVideoAllCTypeApi, getVideoSortListApi, postVideoSortVideoEditApi } from '@/request'
...
const EditModal: React.FC<EditModalPropType> = (P) => {
    ...
    const onFinish = () => {
        form.validateFields().then((v: any) => {
            ...
            postVideoSortVideoEditApi(dataSource.id, { sort: v.sort }).then((res: any) => {
...
export const ContentSortSetting: React.FC = () => {
    ...
    const onDelete = (id: number) => {
        delVideoSortVideoApi(id).then((res: any) => {
            if (res.code == 200) {
    ...
    const initData = () => {
        ...
        getVideoSortListApi(obj.id, { page, per_page, ...forQueryParam }).then((res: any) => {
