...
import { delVideoRecommendApi, getVideoAllSiteApi, getVideoRecommendListApi, getVideoRecommendStateApi, postVideoRecommendAddApi, postVideoRecommendEditApi } from '@/request'
...
export const ContentSortList: React.FC = () => {
    ...
    const onManage = (item: any) => {
        navigate(`/Video/SiteMgr/ContentSortList/Setting?id=${item.id}`, { replace: false })
    }
    const handleDel = (id: number) => {
        delVideoRecommendApi(id).then((res: any) => {
            if (res.code == 200) {
    ...
    const changeStatus = (id: number) => {
        getVideoRecommendStateApi(id).then((res: any) => {
            if (res.code == 200) {
                message.success(res.message)
                initData()
            } else {
    ...
    const modalConfirm = (params) => {
        const operationApi = params.id ? postVideoRecommendEditApi : postVideoRecommendAddApi
    ...
    const initData = () => {
        ...
        getVideoRecommendListApi({ page: page, per_page: per_page, ...searchParamsData }).then((res: any) => {
            if (res.code == 200) {
