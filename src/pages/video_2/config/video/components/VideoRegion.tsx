...
import { getVideoAreasListApi_2, addVideoAreasApi_2, updateVideoAreasApi_2, delVideoAreasApi_2 } from '@/request'
    ...
    return (
        <Modal
            ...
            <Form
                ...
                <Form.Item label="地区名称" name="area_name" rules={[{ required: true, message: '请输入地区名称' }]}>
                    <Input
                        placeholder="请输入地区名称"
                        allowClear />
                </Form.Item>
...
export const VideoRegion: React.FC = () => {
    ...
    const handleDel = (id: number) => {
        delVideoAreasApi_2(id).then((res: any) => {
            if (res.code == 200) {
    ...
    const modalConfirm = (params: BaseDataType) => {
        const operationApi = params.id ? updateVideoAreasApi_2 : addVideoAreasApi_2
    ...
    const initData = () => {
        ...
        getVideoAreasListApi_2({ page, per_page }).then((res: any) => {
            if (res.code == 200) {
