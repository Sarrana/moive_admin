...
import { delPayTypeApi_2, getPayTypeListApi_2, updatePayTypeApi_2 } from '@/request/api/video_2/payway'
import { PayTypeBasedata } from '@/type'
import { Button, Card, Col, message, Row, Space, Switch } from 'antd'
import { useEffect, useMemo, useState } from 'react'
import AddModal from './components/PayTypeAddModal'

export const PayTypeList = () => {
  const [loading, setLoading] = useState<boolean>(false)
  const [dataSource, setDataSource] = useState<PayTypeBasedata[]>([])
  const [total, setTotal] = useState<number>(0)
  const [pageSize, setPageSize] = useState<number>(10)
  const [currentPage, setCurrentPage] = useState<number>(1)
  const [isModalVisible, setIsModalVisible] = useState<boolean>(false)
  const [initItemData, setInitItemData] = useState<PayTypeBasedata>()
  ...
  const paramsData = {
    ...
    columns: [
      ...
      },
      {
        ...
        key: 'operation',
        align: 'center',
        render: (_: any, item: PayTypeBasedata) => (
          <Space>
  ...
  const handleDel = async (id: number) => {
    ...
    try {
      let res: any = await delPayTypeApi_2(params)
      if (res.code == 200) {
  ...
  const getDataList = async () => {
    ...
    try {
      let res: any = await getPayTypeListApi_2(params)
