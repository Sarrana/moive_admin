...
import { delPayGatewayApi_2, getPayGatewayListApi_2, updatePayGatewayApi_2 } from '@/request/api/video_2/payway'
import { PayGatewayBasedata } from '@/type'
import { Button, Card, Col, Image, message, Row, Space, Switch } from 'antd'
import { useEffect, useMemo, useState } from 'react'
import AddModal from './components/PayGatewayAddModal'

export const PayGatewayList = () => {
  const [loading, setLoading] = useState<boolean>(false)
  const [dataSource, setDataSource] = useState<PayGatewayBasedata[]>([])
  const [total, setTotal] = useState<number>(0)
  const [pageSize, setPageSize] = useState<number>(10)
  const [currentPage, setCurrentPage] = useState<number>(1)
  const [isModalVisible, setIsModalVisible] = useState<boolean>(false)
  const [initItemData, setInitItemData] = useState<PayGatewayBasedata>()
  ...
  const paramsData = {
    ...
    columns: [
      ...
      },
      {
        ...
        key: 'img_url',
        align: 'center',
        render: (_, item: any) => (<Image src={item.img_url} width={40} />) 
      },
      {
        title: '名称',
        dataIndex: 'name',
      ...
      },
      {
        ...
        key: 'operation',
        align: 'center',
        render: (_: any, item: PayGatewayBasedata) => (
          <Space>
  ...
  const handleDel = async (id: number) => {
    ...
    try {
      let res: any = await delPayGatewayApi_2(params)
      if (res.code == 200) {
  ...
  const getDataList = async () => {
    ...
    try {
      let res: any = await getPayGatewayListApi_2(params)
