...
import { addCartoonKeyApi, delCartoonKeyApi, editCartoonKeyApi, getCartoonAllSiteApi, getCartoonKeyListApi, getCartoonKeyStateApi } from '@/request'
...
export const KeywordMgr: React.FC = () => {
    ...
    const changeStatus = (id: number) => {
        getCartoonKeyStateApi(id).then((res: any) => {
            if (res.code == 200) {
    ...
    const handleDel = (id: number) => {
        delCartoonKeyApi(id).then((res: any) => {
            if (res.code == 200) {
                message.success(res.message)
                initData()
            } else {
    ...
    const modalConfirm = (params: ParamsDataType) => {
        const operationApi = params.id ? editCartoonKeyApi : addCartoonKeyApi
    ...
    const initData = () => {
        ...
        getCartoonKeyListApi({ page, per_page }).then((res: any) => {
            if (res.code == 200) {
