...
import { getCartoonAllTypeApi, getCartoonUserBookshelfApi, getCartoonUserDetailApi } from "@/request";
...
const CartoonMemberModal = (props: modalPropType) => {
...
const UserData = () => {
    ...
    const getData = () => {
        getCartoonUserDetailApi(obj.id).then((res: any) => {
...
const Bookshelf = () => {
    ...
    const [queryParam, setQueryParam] = useState({ keywords: '', time_1: '', time_2: '', tid: null });
    ...
    const getDataList = () => {
        setLoading(true);
        getCartoonUserBookshelfApi(obj.id, { page: currentPage, per_page: pageSize, ...queryParam }).then((res: any) => {
...
export const CartoonUserDetail: React.FC = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const state = location.state;

    const onBack = () => {
        navigate('/Cartoon/UserMgr/UserList', { state: state, replace: false });
    }
