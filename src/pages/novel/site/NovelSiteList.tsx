...
import { delNovelSiteApi, getNovelSiteListApi, getNovelSiteStateApi } from '@/request';
import { NovelSiteInfo, stateOp } from '@/type';
import SearchForm, { SearchFormItem, SelectOpType } from '@/components/searchForm';
import BaseTable from '@/components/base/BaseTable';
import { NovelSiteListModal } from './NovelSiteListModal';

export const NovelSiteList = () => {
    const [queryParam, setQueryParam] = useState({ name: '', state: null });
    ...
    const [dataSource, setDataSource] = useState<NovelSiteInfo[]>([]);
    ...
    const onDelete = (id: number) => {
        delNovelSiteApi(id).then((res: any) => {
            message.success('删除成功');
            getDataList();
        });
    }
    const onSiteStatusChange = (id: number) => {
        getNovelSiteStateApi({ id: id }).then((res: any) => {
    ...
    const paramsData = {
        ...
        columns: [
            ...
            },
            {
                ...
                dataIndex: 'state',
                key: 'state',
                render: (text: any, item: any) => (<Switch onClick={() => { onSiteStatusChange(item.id) }} checked={`${item.state}` == '1'} />),
                align: 'center'
            },
            {
                title: '操作员',
    ...
    const getDataList = () => {
        setLoading(true);
        getNovelSiteListApi({ page: currentPage, per_page: pageSize, ...queryParam }).then((res: any) => {
